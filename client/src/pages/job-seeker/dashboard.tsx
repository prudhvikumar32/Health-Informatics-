...
const JobSeekerDashboard: React.FC = () => {
  ...
  const topRolesData: TopRoleData[] = [
    { title: 'Chief Medical Information Officer', salary: 145000, width: '100%' },
  ...
  const filteredJobPostings = jobPostingsData.filter(job => {
    const matchesSpecialty = selectedSpecialty === "all_specialties" || 
      (selectedSpecialty === "analytical" && job.title.includes("Analyst") || job.title.includes("Data")) ||
      (selectedSpecialty === "clinical" && job.title.includes("Clinical") || job.title.includes("Medical")) ||
      (selectedSpecialty === "technical" && job.title.includes("Developer") || job.title.includes("Architect") || job.title.includes("Implementation"));
    
    const matchesJobType = selectedJobType === "all_job_types" ||
  ...
  const getJobSpecificSkills = (): SkillData[] => {
    ...
    return skillsData.map(skill => {
      const hasDataRelated = selectedTitles.some(title => 
        title.includes('data') || title.includes('analyst') || title.includes('scientist'));
      
      const hasClinicalRelated = selectedTitles.some(title => 
        title.includes('clinical') || title.includes('ehr') || title.includes('medical'));
        
      const hasTechnicalRelated = selectedTitles.some(title => 
        title.includes('developer') || title.includes('architect') || title.includes('implementation'));
        
      // Adjust percentages based on selected job types
      let adjustedPercentage = skill.percentage;
      
      if (skill.name.includes('Data') || skill.name.includes('SQL') || skill.name === 'Data Analysis') {
        adjustedPercentage = hasDataRelated ? Math.min(98, skill.percentage + 10) : skill.percentage;
      }
      
      if (skill.name.includes('EHR') || skill.name.includes('HL7')) {
        adjustedPercentage = hasClinicalRelated ? Math.min(98, skill.percentage + 10) : skill.percentage;
      }
      
      if (skill.name.includes('Project Management')) {
        adjustedPercentage = hasTechnicalRelated ? Math.min(98, skill.percentage + 10) : skill.percentage;
      }
          ...
          <div className="py-6">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-8">
              <div className="flex flex-col space-y-8">
                ...
                <div>
                  <h1 className="text-2xl font-semibold text-gray-900">Job Seeker Dashboard</h1>
                  <p className="mt-1 text-sm text-gray-600">Welcome back! Here's an overview of the Health Informatics job market.</p>
                </div>
                    ...
                    <div className="flex flex-col md:flex-row gap-4 items-end">
                      <div className="flex-grow">
                        ...
                        >
                          ...
                          <SelectContent>
                            <SelectItem value="all_specialties">All Specialty Areas</SelectItem>
                ...
                <Tabs defaultValue="overview" className="space-y-6">
                      ...
                      {selectedJobs.length > 0 && (
                        <Card>
                          <CardHeader>
                            ...
                            <CardDescription>
                              Insights for your selected jobs ({selectedJobs.length} selected)
                            </CardDescription>
                          </CardHeader>
                          <CardContent>
                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                              ...
                              <div className="space-y-4">
                                <h3 className="text-base font-medium">Key Insights for Selected Jobs</h3>
                                
                                <div className="bg-blue-50 p-4 rounded-md">
                                  <h4 className="font-medium text-blue-700">Salary Range</h4>
                                  <p className="mt-1 text-sm text-blue-600">
                                    The selected roles have an average salary range of $85,000 - $115,000, which is 12% above the national average for health informatics positions.
                                  </p>
                                </div>
                                
                                <div className="bg-green-50 p-4 rounded-md">
                                  <h4 className="font-medium text-green-700">Growth Trajectory</h4>
                                  <p className="mt-1 text-sm text-green-600">
                                    These roles are projected to grow 15-18% over the next five years, creating approximately 3,500 new positions nationally.
                                  </p>
                                </div>
                                
                                <div className="bg-purple-50 p-4 rounded-md">
                                  <h4 className="font-medium text-purple-700">Remote Work Opportunity</h4>
                                  <p className="mt-1 text-sm text-purple-600">
                                    {selectedJobs.some(job => job.title.toLowerCase().includes('analyst') || job.title.toLowerCase().includes('data')) 
                  ...
                  <TabsContent value="skills" className="space-y-6">
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                      ...
                      <TopSkillsChart 
                        skills={skillsData.sort((a, b) => b.percentage - a.percentage)} 
                      ...
                      <Card>
                        ...
                        <CardContent>
                          <div className="space-y-6">
                            <div className="space-y-4">
                              <h4 className="text-sm font-medium text-gray-700">Fastest Growing Skills</h4>
                              <div className="space-y-3">
                                <div className="flex items-center">
                                  <div className="w-32 text-sm">AI/ML in Healthcare</div>
                                  <div className="flex-1 h-2 bg-gray-200 rounded-full overflow-hidden">
                                    <div className="h-full bg-green-500 rounded-full" style={{ width: '85%' }}></div>
                                  </div>
                                  <div className="w-16 text-right text-sm font-medium text-green-600">+35%</div>
                                </div>
                                <div className="flex items-center">
                                  <div className="w-32 text-sm">FHIR Standards</div>
                                  <div className="flex-1 h-2 bg-gray-200 rounded-full overflow-hidden">
                                    <div className="h-full bg-green-500 rounded-full" style={{ width: '70%' }}></div>
                                  </div>
                                  <div className="w-16 text-right text-sm font-medium text-green-600">+28%</div>
                                </div>
                                <div className="flex items-center">
                                  <div className="w-32 text-sm">Cloud Healthcare</div>
                                  <div className="flex-1 h-2 bg-gray-200 rounded-full overflow-hidden">
                                    <div className="h-full bg-green-500 rounded-full" style={{ width: '60%' }}></div>
                                  </div>
                                  <div className="w-16 text-right text-sm font-medium text-green-600">+24%</div>
                                </div>
                              </div>
                            </div>
                            
                            <div className="space-y-4">
                              <h4 className="text-sm font-medium text-gray-700">Essential Skills By Role</h4>
                              <div className="space-y-3">
                                <div className="bg-blue-50 p-3 rounded">
                                  <div className="font-medium text-blue-700">Data Analyst</div>
                                  <div className="mt-1 text-sm text-blue-600">SQL, R/Python, Tableau, Statistical Analysis</div>
                                </div>
                                <div className="bg-green-50 p-3 rounded">
                                  <div className="font-medium text-green-700">Clinical Informatics</div>
                                  <div className="mt-1 text-sm text-green-600">EHR Systems, Clinical Workflow, HL7/FHIR, Medical Terminology</div>
                                </div>
                                <div className="bg-purple-50 p-3 rounded">
                                  <div className="font-medium text-purple-700">IT Implementation</div>
                                  <div className="mt-1 text-sm text-purple-600">Project Management, Change Management, Systems Integration, Training</div>
                                </div>
                              </div>
                            </div>
                          </div>
